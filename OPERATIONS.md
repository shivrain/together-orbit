...
Set `kind` to one of `This month`, `Research breakthrough`, `Industry research`, `Buyer data` (the list in `contentKinds`, `src/platform-types.ts`), and write an `engagementMove` saying what the partner actually does with it. Verify original publication dates and URLs by fetching the page; re-read every quote against the source rather than trusting a summarizer, which has silently inverted a quote before. Where a page shows only a month, either find a day-precision date on the publisher's index or leave the item out — never invent one. Add at most six items with factual summaries, specific relevance, existing company IDs, 2–3 quoted takeaways, a discussion question, source links, and recipient-specific email drafts. Every mapped company should have at least one named recipient angle, grounded in public professional context. Do not invent a private relationship or company pain point. Follow FeedItem in `src/platform-types.ts`. Reuse stable IDs per source to avoid duplicates. Do not invent relationships or recipients.

### Harvesting a month of launches

`This month` items are gathered by fanning research agents out one beat at a time — a beat is a slice like agent infrastructure, security and identity, India, evals, or music and audio — rather than by asking one agent for everything. Each agent returns `{title, org, url, date, what, why, quote, relevantTo}` per item and is told to fetch the announcement page itself; a beat that could not reach a vendor's site must say so rather than fall back on secondary coverage. Search budgets run out partway through a sweep, so expect later beats to work from changelog and newsroom index pages directly.

Three rules decide what survives: the date must be day-precision and inside the month (a page showing only a month is dropped, and the build script refuses anything before the first), the quote must appear verbatim on the fetched page, and the URL must be unique across the whole feed — two beats citing one changelog is the most common duplicate. Cross-check any quote that carries a strong claim; a summarizing agent has inverted one before. Keep the per-beat JSON files, since the merge is rerun from the full set each time and the previous merge's output must be restored from git first, or every item reads as a duplicate.
