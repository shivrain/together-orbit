...
import {type Contact,type MailDraft,type FeedItem,type Movement,type PlatformData,type ReferralOpportunity} from './platform-types';
import {givenName,resourceBody,resourceSubject} from './engagement-message';
import {matchReferrer,referralRequest,requestOptions,type RequestKind} from './referral-workflow';
...
const route=()=>location.hash==='#refer'?'refer':location.hash==='#referrals'?'referrals':location.hash==='#activity'?'activity':'content';
...
export default function Workspace(){
 ...
 const [person,setPerson]=useState<Contact|null>(null),[draft,setDraft]=useState<MailDraft|null>(null);
 ...
 useEffect(()=>{
  ...
  Promise.resolve(context.registerTool({name:'show_sourcing_workspace',description:'Open the Content, Leads or Activity workspace.',inputSchema:{type:'object',properties:{section:{type:'string',enum:['content','referrals','activity']}},required:['section'],additionalProperties:false},execute:async({section}:{section:'content'|'referrals'|'activity'})=>{location.hash=section;setPage(section);return {section}}},{signal:controller.signal})).catch(()=>{});
 ...
 function askAboutMove(p:Contact,move:Movement){
  const moved=move.type==='Started company'
   ?`I saw that ${move.person} has started ${move.currentCompany||'something new'}.`
   :`I saw that ${move.person} has moved to ${move.currentCompany||'a new role'}.`;
  setPerson(p);
  setDraft({id:newId('draft'),companyId:p.companyId,personId:p.id,to:p.email,subject:`${move.person}`,
   body:`Hi ${givenName(p.name)},\n\n${moved} ${move.previousRole?`They were ${move.previousRole} with you.`:'They used to be on your team.'}\n\nWould you be willing to tell me what they were like to work with, and whether you think we should meet them? No pressure if it is awkward.\n\nSource I saw: ${move.sourceUrl}`,
   status:'Draft',createdAt:new Date().toISOString(),purpose:'referral-ask'});
  setModal('compose');
 }
 ...
 const titles:Record<string,string>={compose:draft?.purpose==='referral-ask'?'Ask for a referral':'Prepare a message',request:'Ask for introductions',share:'A simple way to share a founder',settings:'Workspace settings'};
 return <div className="sw fw">
  <Toaster position="bottom-left" richColors/>
  <header className="sw-topbar"><a className="sw-brand" href="#content" onClick={e=>{e.preventDefault();navigate('content')}}>together<span>FUND</span></a><span className="sw-product">Portfolio sourcing</span><nav aria-label="Workspace"><button aria-current={page==='content'?'page':undefined} className={page==='content'?'active':''} onClick={()=>navigate('content')}>Content</button><button aria-current={page==='referrals'?'page':undefined} className={page==='referrals'?'active':''} onClick={()=>navigate('referrals')}>Leads</button><button aria-current={page==='activity'?'page':undefined} className={page==='activity'?'active':''} onClick={()=>navigate('activity')}>Activity</button></nav><button className="sw-icon" aria-label="Workspace settings" onClick={()=>setModal('settings')}><Settings2 size={18}/></button></header>
  {error&&<div className="sw-error">{error}</div>}
  <main>{page==='content'?<ContentWorkbench data={data} onCompose={compose} onOpenDraft={openDraft}/>
   :page==='activity'?<ActivityWorkbench data={data} onAsk={askAboutMove}/>
   :<ReferralWorkbench data={data} onRequest={requestReferrals} onShareForm={()=>setModal('share')} onOpenDraft={openDraft} onAskOpportunity={askAboutOpportunity}/>}</main>
  <Dialog open={!!modal} onOpenChange={open=>{if(!open)setModal('')}}><DialogContent className={`sw-dialog ${modal==='compose'?'sw-compose':''}`}><DialogHeader><DialogTitle>{titles[modal]}</DialogTitle><DialogDescription>{modal==='compose'?'Review the message, then copy it or open your email app.':modal==='request'?'Choose someone to ask and make the request specific.':modal==='share'?'Founders fill this out to prepare an email introduction to Together.':'Referral address and your saved work.'}</DialogDescription></DialogHeader>

  {modal==='compose'&&draft&&<>
   <div className="sw-composer-context"><strong>{person?.name||'Recipient'}</strong><span>{companyName(draft.companyId)}</span></div>
